...
async def main():
    ...
    query = "SELECT ticker FROM tickerstable WHERE ticker = 'A' "
    ...
    for _ , row in ticker_data.iterrows():
        ticker = row['ticker']
        resp = await stocks_client.get_aggregate_bars(symbol=ticker, multiplier=1, timespan='day', from_date='2024-07-25', to_date='2024-07-27')
        if 'results' in resp:
    ...
    try:
        data_to_insert.to_sql('polygondata', con=engine, if_exists='replace', index=False)
        print("Data Inserted")
